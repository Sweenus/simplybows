...
public class CosmicStrikeVisualEntityRenderer extends EntityRenderer<CosmicStrikeVisualEntity> {
    ...
    private static final float LIFETIME_TICKS = 7.0F;
    ...
    public void render(CosmicStrikeVisualEntity entity, float yaw, float tickDelta, MatrixStack matrices,
        ...
        float alpha = Math.max(0.0F, 1.0F - (entity.age + tickDelta) / LIFETIME_TICKS);
        if (alpha <= 0.02F) {
            return;
        }
        ...
        for (int i = 1; i < POINTS; i++) {
            ...
            CosmicOrbitVisualEntityRenderer.renderLine(matrices, lineConsumer, lastX, lastY, lastZ, x, y, z, alpha);
            CosmicOrbitVisualEntityRenderer.renderNodeDisc(matrices, nodeConsumer, nodeSprite, x, y, z, 0.045F * alpha, alpha);
            lastX = x;
            lastY = y;
            lastZ = z;
        }
